...
# Cấu hình DB
DATABASE_URL = os.getenv("DATABASE_URL", "postgresql://aegis_user:aegis_secret@localhost:5432/aegis_db")
...
def process_image(task_id: str, image_path: str):
    ...
    try:
        ...
        similar_products = db.query(Product).filter(
            Product.vector_embedding.is_not(None)
        ).order_by(
            Product.vector_embedding.cosine_distance(img_emb)
        ).limit(3).all()
