...
def get_nearby_places(db: Session, lat: float, lon: float, radius_meters: int = 2000):
    ...
    query = db.query(Place, ST_Distance(Place.geom.cast('geography'), user_location.cast('geography')).label('distance')) \
            .filter(ST_DWithin(Place.geom.cast('geography'), user_location.cast('geography'), radius_meters)) \
            .order_by(ST_Distance(Place.geom.cast('geography'), user_location.cast('geography'))).limit(50)
    results = query.all()
...
async def plan_route_osrm(db: Session, current_lat: float, current_lon: float, store_ids: list[int]):
    stores = db.query(Store).filter(Store.id.in_(store_ids)).all()
    if not stores: raise ValueError("Không có Data Cửa hàng khớp với store_ids cung cấp")
         
    weather = await fetch_real_weather(current_lat, current_lon)
    store_dicts = [{"id": s.id, "lat": s.lat, "lon": s.lon} for s in stores]
    
    # 1. Tính toán lộ trình TSP bằng thuật toán Tham Lam (Khoảng cách Euclid cơ bản)
    optimized_stores = calculate_tsp_greedy(current_lat, current_lon, store_dicts)
    optimized_order_ids = [s["id"] for s in optimized_stores]
    
    coords = [f"{current_lon},{current_lat}"]
    for s in optimized_stores: coords.append(f"{s['lon']},{s['lat']}")
    ...
    try:
        ...
        # Fallback khẩn cấp nếu OSRM Map bị sập Timeout:
        # Trả về đường chim bay (직선 거리) để App không Crash và tiếp tục chuỗi O2O
        return {
            "total_distance_meters": 0.0,
            "waypoints": [],
            "polyline": None, # App Frontend sẽ tự nhả straight-line khi None
            "optimized_order": optimized_order_ids,
            "weather_context": weather
        }