...
def add_virtual_closet(
    ...
    item = service.add_to_closet(db=db, user_id=user_id, image_path=file_path)
    return item


@router.get("/closet", response_model=list[schema.ClosetItemResponse])
def my_closet(user_id: int = Depends(get_current_user), db: Session = Depends(get_db)):
    return service.get_user_closet(db=db, user_id=user_id)
