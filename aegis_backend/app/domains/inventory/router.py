...
async def create_inventory_lock(request: schema.LockRequest, user_id: int = Depends(get_current_user), db: Session = Depends(get_db), redis: Redis = Depends(get_redis)):
    """API Phân tầng: Chạm phanh Redis trước, Lock Postgres sau. An toàn giữ hàng O2O."""
    lock = await service.create_lock(db=db, redis=redis, request=request, user_id=user_id)
...
async def get_my_locks(user_id: int = Depends(get_current_user), db: Session = Depends(get_db), redis: Redis = Depends(get_redis)):
    """Tra cứu Giỏ hàng & Đồng hồ đếm ngược được nuôi bởi Redis"""
    return await service.get_user_locks_with_ttl(db=db, redis=redis, user_id=user_id)
